...
function handleScroll() {
 ...
 if (activeIndex!==activeIndex_){
 	...
 	if((activeIndex===-Infinity||activeIndex===Infinity||activeIndex===undefined)&&(activeIndex_>-Infinity&&activeIndex_<Infinity)){
   ...
   .style("right", document.getElementsByTagName("body")[0].getBoundingClientRect().right+ parseInt(bodyMarginPx)- containerRight+parseInt(containerBorderWidthPx)+"px");
     ...
     if (activeIndex_===-1){
      ...
      self.transitions.inactiveTransition?self.transitions.inactiveTransition():0;

     }
 	} else if (activeIndex_===-Infinity||activeIndex_===Infinity){
